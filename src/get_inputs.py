...
def get_inputs(args):
    ...
    if args.dataset in ['nonliving26', 'living17', 'entity13', 'entity30']:
        ...
        features, labels, outputs, indices = prep_data(data, args.dataset, idir)
        ret = eval(f"make_{args.dataset}")(args.data_dir, split='good')
        ...
        features, labels, outputs, indices = data["features"], data["labels"], data["outputs"], data["indices"]
        datad = Imagenet_Folder_with_indices(f'{args.data_dir}/officehome/{args.domain}/')
        _, breeds_idx = datad.find_classes(f'{args.data_dir}/officehome/{args.domain}/')
        breeds_idx = {" ".join(k.split("_")): v for k,v in breeds_idx.items()}
        super_classes = list(breeds_idx.keys())

    elif args.dataset == 'fashion1m':
        data = np.load(features_file % 'fashion1m')
        features, labels, outputs, indices = data["features"], data["labels"], data["outputs"], data["indices"]
        datad = Imagenet_Folder_with_indices(f'{args.data_dir}/fashion1M/')
        _, breeds_idx = datad.find_classes(f'{args.data_dir}/fashion1M/')
        breeds_idx = {" ".join(k.split("_")): v for k,v in breeds_idx.items()}
        super_classes = list(breeds_idx.keys())
    ...
    if args.experiment in ['true', 'true_lin']:
        assert args.dataset in TRUESETS
        return features, labels, sup2sub, super_classes
    elif args.experiment in ['gpt', 'gpt_lin']:
        if args.rerun_gpt:
            syn_d = get_cleaned_gpt_sets(super_classes, args.label_set_size, temp=args.temp)
        ...
        return features, labels, syn_d, super_classes
    else:
